...
def getRecoverScene(img, omega=0.95, t0=0.1, blockSize=15, meanMode=False, percent=0.001):
    ...
    # Ã©ËœÂ²Ã¦Â­Â¢Ã¥â€¡ÂºÃ§Å½Â°tÃ¥Â°ï¿½Ã¤ÂºÅ½0Ã§Å¡â€žÃ¦Æ’â€¦Ã¥â€ Âµ
    # Ã¥Â¯Â¹tÃ©â„¢ï¿½Ã¥Ë†Â¶Ã¦Å“â‚¬Ã¥Â°ï¿½Ã¥â‚¬Â¼Ã¤Â¸Âº0.1
    ...
    for i in range(len(files)):
        ...
        if os.path.isfile(filepath):
            ...
            if(int(tag)):
                ...
                if (int(firstCall)):
                    cv2.putText(sceneRadiance, '_DCP', (0,h-10) ,cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 255, 255), 2 )
                else:
                    cv2.putText(sceneRadiance, '_DCP', (0,h-30) ,cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 255, 255), 2 )

            
            #cv2.imwrite(os.path.join(folder,'OutputImages',(prefix + '_DCP_TM.jpg')), np.uint8(transmission * 255))
            cv2.imwrite(os.path.join(folder,'OutputImages',(prefix + '_DCP.jpg')), sceneRadiance)
