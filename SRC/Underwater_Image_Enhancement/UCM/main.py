    ...
    for i in range(len(files)):
        ...
        if os.path.isfile(filepath):
            ...
            if(int(tag)):
                ...
                if (int(firstCall)):
                    cv2.putText(sceneRadiance, '_UCM', (0,h-10) ,cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 255, 255), 2 )
                else:
                    cv2.putText(sceneRadiance, '_UCM', (0,h-30) ,cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255, 255, 255), 2 )

            cv2.imwrite(os.path.join(folder,'OutputImages',(prefix + '_UCM.jpg')), sceneRadiance)
